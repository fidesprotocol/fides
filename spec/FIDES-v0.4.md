...
**Registration Delay Rule (6.4.4):** The maximum allowed time between `decision_date` and `record_timestamp` is defined by the parameter `MAX_REGISTRATION_DELAY`.
- Implementations MUST define this parameter.
- The parameter MUST NOT exceed 90 days (Hardware Limit).
Decisions not registered within this window require a Special Decision Record (SDR) with `exception_type: late_registration` and formal justification. This prevents silent backdating of decisions.
...
**Cooldown Rule:**
If a new SDR is requested with the same `exception_type` and `authority_id` as a previous SDR contained within the last 365 days, the implementation MUST enforce a waiting period or increased quorum requirement.

*Recommended Formula: `Cooldown = BASE_COOLDOWN * (2 ^ consecutive_occurrences)`.*
...
2.  **Involuntary Assignment (Judicial/Administrative):**
    ...
    - **Rationale:** The Authority acknowledges the Court Order and commits to redirecting the payment. The protocol validates the Authority's signature because the Authority is a known actor in the DR, whereas a random "Judge" key is unverifiable without a global Judiciary PKI.
    - **Role:** The Authority acts as the "Oracle" for the physical court order.
...
3.  **Permanent Stickiness:** The DR and all subsequent events are permanently marked as "Tainted".
4.  **Audit Flag:** Generates a CRITICAL audit alert that cannot be cleared.
...
**Note on system_parameters:** This field is REQUIRED for `act_type: genesis` and OPTIONAL (informational) for others. Typically, it is null for standard records.
