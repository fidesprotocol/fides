...
#### 6.4.5 Registration Delay Audit Trail (Critical)

To prevent abuse of the 72-hour window, all registrations with delay > 1 hour MUST include additional justification.

**Delay Tiers and Requirements:**

| Delay | Classification | Additional Requirements |
|-------|----------------|------------------------|
| 0-1 hour | Normal | None |
| 1-24 hours | Delayed | `delay_reason` field required |
| 24-72 hours | Late | `delay_reason` + supervisor approval signature required |
| > 72 hours | Expired | MUST use SDR with `exception_type: late_registration` |

**Mandatory Fields for Delayed Registration (> 1 hour):**

```json
{
  "delay_justification": {
    "delay_hours": "number (calculated: record_timestamp - decision_date)",
    "delay_reason": "enum (system_outage | weekend_closure | holiday | manual_processing | other)",
    "delay_explanation": "string (minimum 100 characters if delay_reason is 'other')",
    "supervisor_approval": {
      "// Required only if delay > 24 hours": "",
      "supervisor_id": "string",
      "supervisor_signature": "object (same format as signatures)",
...
1. **Delay Statistics:** Technical Auditor MUST publish monthly statistics on registration delays
2. **Anomaly Detection:** Patterns of consistent 71-hour delays indicate policy circumvention
3. **Trend Analysis:** Increasing average delay times MUST trigger oversight review
4. **Per-Authority Tracking:** Delay statistics MUST be tracked per `authority_id`

**Red Flags (require investigation):**
- Authority consistently registering at 70-72 hours
- Sudden spike in delayed registrations before holidays
- High percentage of "other" delay reasons
- Supervisor approving own delayed registrations

**Rationale:** Without visibility into registration delays, the 72-hour window becomes a systematic backdating mechanism. Recording why delays occur and tracking patterns enables detection of abuse.
...
4. **Guardian Plurality (Optional but Recommended):**
   - Implementations SHOULD have multiple Immutability Guardians
   - Each Guardian independently publishes to their assigned media
   - Minimum 2-of-3 anchor agreement required for validity
...
To prevent the normalization of exceptions (e.g., renewing a "State of Calamity" indefinitely), the protocol imposes an algorithmic cooldown cost.

**Exponential Cooldown Rule:**
If a new SDR is requested with the same `exception_type` and `authority_id` as a previous SDR contained within the last 365 days:

1.  **Waiting Period:** A mandatory cooldown period applies before the new SDR becomes valid.
    *   Formula: `Cooldown = 30 days * (2 ^ previous_consecutive_SDRs)`
    *   Example: 1st renewal = 30 days wait. 2nd renewal = 60 days wait. 3rd renewal = 120 days.
2.  **Bypass Quorum:** The cooldown can only be bypassed by a "Super-Majority" signature set:
    *   Requires 80% of all registered deciders (vs simple majority)
    *   Requires explicit approval from `oversight_authority`
...
#### 10.3.2 Organization Chart Versioning (Critical)

To prevent bureaucratic manipulation of organization charts, all chart updates MUST be externally anchored and versioned.

**Mandatory Requirements:**

1. **Immutable Chart History:**
   - Organization charts MUST be versioned (v1, v2, v3...)
   - Each version MUST have an external timestamp proof (RFC 3161 or blockchain)
   - Old versions MUST remain accessible at their original URLs
   - Deletion or modification of old versions is PROHIBITED

2. **Version Anchoring:**
   - Every chart update MUST be anchored within 24 hours of publication
   - The anchor MUST include:
     - `chart_version`: Sequential version number
     - `chart_hash`: SHA-256 of the chart content
     - `valid_from`: DateTime when this version became effective
     - `previous_version_hash`: Hash of the previous version (if any)

3. **Chart Change Constraints:**
   - Changes affecting reporting relationships MUST have a 7-day notice period
   - Notice MUST be published on the same URL with a `pending_changes` section
   - Expedited changes (< 7 days) require oversight authority approval

4. **Revocation Time Binding:**
   - For hierarchical superior revocations, the chart version used MUST be:
     - The version that was valid at the time of the original DR, OR
     - A version published BEFORE the original DR
   - Using a chart version published AFTER the original DR is INVALID

5. **Chart Accessibility:**
   - Charts MUST be publicly accessible without authentication
   - Charts MUST be machine-readable (JSON format)
   - Historical versions MUST be retrievable by version number

**Rationale:** Without strict versioning, a government can retroactively modify organization charts to enable invalid revocations or block legitimate ones. Mandatory anchoring creates an unforgeable audit trail of organizational changes.
...
4. **Graceful Degradation (DoS Resilience):**
   - If anchor capability is not restored within 72 hours (e.g., global internet outage), the system enters `DEGRADED_MODE`.
   - **Operational Limits in Degraded Mode:**
     *   Maximum value per DR is capped at 10% of the authority's average daily volume (calculated from last 30 days).
     *   "Essential Service" payments remain allowed; all other types suspended.
     *   Physical backup requirement: Deciders MUST sign a physical hash sheet for later reconciliation.
   - **Automatic Restoration:** Mode clears immediately when a valid anchor is confirmed.

   **Rationale:** Prevents a Denial-of-Service attack on the anchor infrastructure from completely paralyzing essential government functions (like paying police or hospitals), while limiting the exposure to fraud during the outage.
